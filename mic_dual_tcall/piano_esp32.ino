...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .btn-delete:hover { background: rgba(244,67,54,0.15); color: #f44336; border-color: rgba(244,67,54,0.3); }
  ...
  <div class="controls">
    ...
    <div class="card">
      ...
      <div style="margin-top: 10px;">
        ...
        <button class="btn btn-delete" id="btnDelete" onclick="deleteMP3()">🗑 Supprimer</button>
      </div>
    </div>
  </div>
</div>
...
<script>
  ...
  notes.forEach((n, idx) => {
    ...
    key.addEventListener('touchstart', (e) => { e.preventDefault(); noteOn(idx); });
    key.addEventListener('touchend',   (e) => { e.preventDefault(); noteOff(idx); });
    key.addEventListener('touchcancel',(e) => { e.preventDefault(); noteOff(idx); });
    key.addEventListener('mousedown',  (e) => { e.preventDefault(); noteOn(idx); });
    key.addEventListener('mouseup',    () => noteOff(idx));
    key.addEventListener('mouseleave', () => { if (activeNotes.has(idx)) noteOff(idx); });
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (idx !== undefined && !activeNotes.has(idx)) noteOn(idx);
  });
  ...
  function deleteMP3() {
    if (!confirm('Supprimer le fichier MP3 sur l\'ESP32 ?')) return;
    stopMP3();
    fetch('/delete', { method: 'POST' }).then(r => {
      if (r.ok) {
        document.getElementById('fileInfo').textContent = '🗑 Fichier supprimé';
        document.getElementById('btnPlay').disabled = true;
        document.getElementById('btnPlay').textContent = '▶ Jouer';
        document.getElementById('progressFill').style.width = '0%';
        document.getElementById('fileInput').value = '';
      } else {
        document.getElementById('fileInfo').textContent = '❌ Erreur suppression';
      }
    });
  }
</script>
...
void tickNote() {
  ...
  for (int i = 0; i < 64; i++) {
    ...
    if (!audioOut->ConsumeSample(ms)) break;

    if (_noteActive || _noteFadingOut) _notePos++;
  }
}
...
void setup() {
  ...
  server.on("/delete", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    if (SPIFFS.exists("/uploaded.mp3")) {
      ...
      Serial.println("🗑 MP3 supprimé");
      request->send(200, "text/plain", "OK");
    } else {
      request->send(404, "text/plain", "Pas de fichier");
    }
  });
  ...
  Serial.println(" dans votre navigateur!");
  Serial.println("============================================\n");
}
